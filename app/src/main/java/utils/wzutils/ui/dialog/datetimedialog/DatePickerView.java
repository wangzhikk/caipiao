...
public class DatePickerView extends View {
    ...
    private Paint paintSelected, paintNormal;
    ...
    public String getCurrSelected(){
        if(mDataList.size()>mCurrentSelected)return mDataList.get(mCurrentSelected);
        return "";
    }
    ...
    private void init() {
        ...
        paintSelected.setColor(textSelectedColor);
        //第二个paint
        paintNormal = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintNormal.setStyle(Style.FILL);
        paintNormal.setTextAlign(Align.CENTER);
    ...
    private void drawData(Canvas canvas) {
        ...
        paintSelected.setTextSize(size);
        paintSelected.setAlpha((int) ((mMaxTextAlpha - mMinTextAlpha) * scale + mMinTextAlpha));
        // text居中绘制，注意baseline的计算才能达到居中，y值是text中心坐标
        float x = (float) (mViewWidth / 2.0);
        float y = (float) (mViewHeight / 2.0 + mMoveLen);
        FontMetricsInt fmi = paintSelected.getFontMetricsInt();
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));

        canvas.drawText(mDataList.get(mCurrentSelected), x, baseline, paintSelected);
        // 绘制上方data
    ...
    private void drawOtherText(Canvas canvas, int position, int type) {
        ...
        float size = (mMaxTextSize - mMinTextSize) * scale + mMinTextSize;
        paintNormal.setTextSize(size);
        paintNormal.setAlpha((int) ((mMaxTextAlpha - mMinTextAlpha) * scale + mMinTextAlpha));
        float y = (float) (mViewHeight / 2.0 + type * d);
        FontMetricsInt fmi = paintNormal.getFontMetricsInt();
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));
        canvas.drawText(mDataList.get(mCurrentSelected + type * position),
                (float) (mViewWidth / 2.0), baseline, paintNormal);
    }
